...
properties([
    parameters([
      choice(name: 'RELEASE',
             choices: ["4.21-10.1-ocp4nv"],
             description: 'Node image stream to build'),
      string(name: 'FROM',
...
lock(resource: "build-node-image") {
    ...
    timeout(time: 45, unit: 'MINUTES') {
        ...
        stage('Build Node Image') {
            withCredentials([file(credentialsId: 'oscontainer-push-registry-secret-nvidia', variable: 'REGISTRY_AUTH_FILE')]) {
                 ...
                 node_image_manifest_digest = pipeutils.build_and_push_image(arches: arches,
                                                ...
                                                extra_build_args: ["--security-opt label=disable", "--mount-host-ca-certs", "--force",
                                                                   "--add-openshift-build-labels", "--git-sub-dir", "ocp4nv"] + label_args)
            }
        }
        if (!skip_brew_upload){
            stage("Brew Upload") {
